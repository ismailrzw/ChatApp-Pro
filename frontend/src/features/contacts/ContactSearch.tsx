  ...
  const handleAddContact = async (uid: string) => {
    ...
    try {
      ...
      if (err.response?.status === 409) {
        setRequested((prev) => ({ ...prev, [uid]: true }))
        showToast('Already connected or pending', 'info')
      } else {
          ...
          <div className="text-center py-12 animate-in fade-in zoom-in duration-300">
            <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-slate-100">
              <svg className="w-8 h-8 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.172 9.172a4 4 0 0112.728 0M5.636 5.636a9 9 0 0114.142 0M12 14a2 2 0 110-4 2 2 0 010 4z" />
              </svg>
            </div>
