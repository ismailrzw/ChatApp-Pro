...
export default function ContactList({ isSidebarMode = false }: ContactListProps) {
  const { contacts, setContacts, loading, setLoading, setError, removeContact, addContact } = useContactsStore()
  const [isSearchOpen, setIsSearchOpen] = useState(false)
  const [blockModal, setBlockModal] = useState<{ isOpen: boolean; contactId: string; name: string }>({
    isOpen: false,
    contactId: '',
    name: '',
  })
  ...
  const handleBlock = async () => {
    try {
      ...
      showToast(`${blockModal.name} blocked`, 'success')
    } catch (err) {
  ...
  const handleAcceptRequest = async (request: any) => {
    try {
      ...
      addContact(updated)
      setIncomingRequests((prev) => prev.filter((r) => r.contact_id !== request.contact_id))
  ...
  return (
    <div className="flex flex-col h-full bg-white relative min-h-0">
      ...
      <div className={`
        ...
        ${isSidebarMode ? 'p-4 border-b border-slate-50 mt-2' : 'p-6 md:p-10 border-b border-slate-100'}
      `}>
        <div>
          <h1 className={`${isSidebarMode ? 'text-lg' : 'text-3xl'} font-black text-slate-900 tracking-tight`}>
            Contacts
          </h1>
        ...
        <button 
          ...
          className={`
            flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white transition-all active:scale-95 shadow-lg shadow-blue-100
            ${isSidebarMode ? 'w-10 h-10 rounded-xl' : 'px-6 py-3 rounded-2xl font-black text-sm group'}
          `}
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4" />
          </svg>
          {!isSidebarMode && <span className="ml-2 hidden sm:inline">Add Contact</span>}
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar min-h-0">
        ...
        {incomingRequests.length > 0 && (
          <div className={`bg-blue-50/50 border-b border-blue-100 space-y-3 animate-in fade-in duration-500 ${isSidebarMode ? 'p-4' : 'px-6 md:px-10 py-6'}`}>
            <h3 className="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] ml-1">New Requests</h3>
            <div className={`grid gap-3 ${!isSidebarMode ? 'grid-cols-1 sm:grid-cols-2' : ''}`}>
              {incomingRequests.map((req) => (
                <div key={req.contact_id} className="flex items-center justify-between bg-white p-3 rounded-2xl shadow-sm border border-blue-100/50">
                  <div className="flex items-center gap-3 min-w-0">
                    <Avatar src={req.from_user.avatar_url} name={req.from_user.display_name} size="sm" />
          ...
          <div className="divide-y divide-slate-50 px-2">
            {sortedContacts.map((contact) => (
              <ContactCard 
                ...
                onClick={(uid) => {
                  showToast('Messaging coming in Sprint 3', 'info')
                }}
                onBlock={(id) => setBlockModal({ 
                  isOpen: true, 
                  contactId: id, 
      ...
      >
        <p className="text-slate-500 text-sm font-medium">This will prevent further messages and requests.</p>
      </Modal>
