...
export default function ContactList({ isSidebarMode = false }: ContactListProps) {
  ...
  // showToastRef: keeps latest showToast without putting it in dep arrays.
  // Hook functions recreate every render — refs are always the same object.
  ...
  // Empty []. No functions, no Zustand actions in deps — would cause infinite loop.
  // currentUser read via getState() — reads once at call-time, no subscription.
  useEffect(() => {
    let cancelled = false
  ...
  const handleUnfriend = async () => {
    try {
      ...
      showToastRef.current(
        `${unfriendModal.name} removed from contacts`,
        'success'
      )
    } catch {
  ...
  const handleAcceptRequest = async (request: any) => {
    try {
      const updated = await updateContact(request.contact_id, {
        status: 'accepted',
      })
      addContact(updated)
      setIncomingRequests((prev) =>
  ...
  const sortedContacts = [...contacts].sort((a, b) =>
    (a.other_user?.display_name ?? '').localeCompare(
      b.other_user?.display_name ?? ''
    )
  )
    ...
    <div className="flex flex-col h-full bg-white relative min-h-0">
      ...
      >
        ...
        <button
          ...
          className={`flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white transition-all active:scale-95 shadow-lg shadow-blue-100 ${
            isSidebarMode
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar min-h-0">
        ...
        {incomingRequests.length > 0 && (
          <div
            className={`bg-blue-50/50 border-b border-blue-100 space-y-3 ${
          ...
          >
            ...
            <div
              className={`grid gap-3 ${
                !isSidebarMode ? 'grid-cols-1 sm:grid-cols-2' : ''
              }`}
            >
          ...
          <div className="divide-y divide-slate-50 px-2">
            {sortedContacts.map((contact) => (
              <ContactCard
                ...
                contact={contact}
                onClick={() => {
                  showToastRef.current('Open chat in the Messages tab', 'info')
                }}
                onBlock={(id) =>
                  setBlockModal({
                    isOpen: true,
